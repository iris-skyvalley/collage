...
function drawSelection(ctx: CanvasRenderingContext2D, layer: Layer, scale: number): void {
  ...
  ctx.translate(tr.x * scale, tr.y * scale);
  ctx.rotate(tr.rotation);
  // A hairline and square handles, drawn white under black so they read on
  // dark and light fragments alike. Geometry matches selection.ts exactly.
  ...
  // The rotate handle: a stem from the top edge to a small circle.
  const stem = ROTATE_STEM * scale;
  ctx.beginPath();
  ctx.moveTo(0, -h / 2);
  ctx.lineTo(0, -h / 2 - stem + handle / 2);
  ctx.stroke();
  ctx.beginPath();
  ctx.arc(0, -h / 2 - stem, handle / 2, 0, Math.PI * 2);
  ctx.fill();
  ctx.stroke();
  ctx.restore();
}
