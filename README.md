...
arrows to nudge. With touch: drag to move, pinch to scale and rotate, hold
to remove.
...
- **M0 — Canvas.** Complete. Fixed 4:5, layer manipulation, a 60-asset tray per
  ...
  viewport.
- **M1 — Verbs.** All six are implemented and run locally with no provider;
  the editor offers two of them. Edge is the one the PRD builds its craft
  argument on, and palette sits with the tray. Material, cut, extend and
  relight render on any piece that carries them but are not in the editor:
  material is a style rather than a signal — a filter by another name, which
  Polyvore also never had — and cut, extend and relight are the PRD's "after"
  tier, which without a model behind them read as more rows of controls
  rather than the semantic tools §8.3 describes. They come back when a
  provider does — see *Where the AI goes* in `docs/ARCHITECTURE.md`.
